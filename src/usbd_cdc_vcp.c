...
extern  volatile uint8_t APP_flag;
volatile uint8_t UsbRxBuf[APP_RX_DATA_SIZE];
volatile uint8_t UsbRxLen=0;
...
uint16_t VCP_DataTx (uint8_t* buf, uint32_t len)
{
	...
	if(USB_OTG_dev.dev.device_status == USB_OTG_CONFIGURED){
		...
		//while(APP_free_buff_len < (len+2*CDC_DATA_MAX_PACKET_SIZE)){ //wait for space in APP buff..
			if(toc>i){
				return USBD_FAIL;
			}
		}
